# 📝 Team Contract

**Team Name:** UTOPIA  
**Course:** Product Development for Software Engineers  
**Semester:** Fall 2025  

---

## 👥 Team Members

| Name | Email | Primary Strengths | Preferred Role |
|------|--------|-------------------|----------------|
| Teona Berozashvili | berozashvili.teona@kiu.edu.ge | PM (4), Backend (3), Docs (4) | — |
| Ani Kelenjeridze | kelenjeridze.ani@kiu.edu.ge | Eng (4), PM (3), Doc (4) | — |
| Saba Morchilashvili | morchilashvili.saba@kiu.edu.ge | — | — |
| Aleksi Tkebuchava | tkebuchava.aleksi@kiu.edu.ge | — | — |

---

## 🎯 Team Roles & Responsibilities

### **Tech Lead:** [Name]
**Responsibilities:**
- Oversee technical architecture and technology decisions  
- Manage code quality and review pull requests  
- Coordinate technical work across team members  
- Ensure MVP meets technical requirements  
- Lead technical troubleshooting  

**Backup:** [Name]

---

### **Discovery Lead:** [Name]
**Responsibilities:**
- Manage customer discovery process and user research  
- Coordinate interview scheduling and execution  
- Lead synthesis sessions and define problem statements  
- Maintain interview logs and research documentation  
- Ensure team stays customer-focused  

**Backup:** [Name]

---

### **Program Lead:** [Name]
**Responsibilities:**
- Track deadlines and deliverables  
- Coordinate team meetings and maintain agendas  
- Manage GitHub repository organization  
- Ensure documentation is up to date  
- Communicate with instructor as needed  

**Backup:** [Name]  

> 💡 *Roles can rotate mid-semester if the team agrees.*

---

## ⚖️ Decision-Making Process

### Primary Method: 

**Option A: Consensus** – Everyone must agree before moving forward  
> ⏳ If no consensus within 45 minutes → escalate to vote

**Option B: Majority Vote** – Majority wins; minority records dissent  
> ✅ Quorum: At least 3 members present

**Option C: Role-Based Authority** – Final say based on area  
- Tech → Tech Lead  
- Research → Discovery Lead  
- Process → Program Lead  
- Strategy → Team vote  

**Option D: [Custom Method]**

### Escalation Path
1. Sleep on it → revisit next day  
2. Seek advice from another team  
3. Escalate to instructor  

---

## 💬 Communication Norms

**Primary Channel:** [e.g., Messenger / Teams / Outlook ]  


### Response Time
| Type | Expected Response |
|------|--------------------|
| Urgent | Within 2 hours |
| General | Within 8 hours |
| Weekend | [Response window or “no expectation”] |

### Meeting Schedule
| Type | Frequency | Duration | Location | Notes |
|------|------------|-----------|-----------|--------|
| Regular | [e.g., Tue 6PM & Fri 4PM] | 60–90 min | Online, Campus , K-building | Agenda shared 24h before |
| Emergency | With 12h notice | As needed | — | Include reason for meeting |

### Etiquette
- [ ] Notify team of availability changes  
- [ ] Use @mentions for direct questions  
- [ ] Mark urgent items clearly  
- [ ] Stay respectful and professional  
- [ ] [Add custom team norms]  

---

## 🧱 Work Expectations

**Weekly Contribution:** ~[X] hours per member  

**Tracking Contributions:**  
- [ ] GitHub commits & PRs  
- [ ] Shared task board (Trello / Notion / GitHub Projects)  
- [ ] Weekly standups  
- [ ] Time logs (optional)  
- [ ] Other: [Specify]  

### Quality Standards
- All code reviewed by at least **1–2 teammates**  
- All research deliverables reviewed by Discovery Lead  
- Documentation updated within **24 hours** of changes  

### Accountability Checklist
- [ ] Attend meetings (notify 12+ hours in advance if absent)  
- [ ] Complete assigned tasks by deadlines  
- [ ] Communicate blockers early  
- [ ] Provide constructive feedback  
- [ ] Maintain professional conduct  

---

## ⚔️ Conflict Resolution

### Process
1. **Direct Talk (24–48h):** Private, respectful discussion  
2. **Mediation:** Neutral team member facilitates if Step 1 fails  
3. **Escalate:** Instructor mediation if still unresolved  

### Handling Common Issues

**Missed Deadlines:**  
→ Explain reason → Redistribute work → Adjust future planning  

**Unequal Contribution:**  
→ Address privately → Discuss at meeting → Adjust workloads  

**Technical Disagreements:**  
→ Tech Lead researches → Team discusses → Vote if needed  

**Personal Conflicts:**  
→ Use 3-step process above; prioritize respect  

---

## ✍️ Signatures & Agreement

By signing, we commit to:
- Uphold this contract through the semester  
- Hold ourselves and others accountable  
- Revisit and update by mutual agreement  
- Support our collective success  

| Member | Signature | Date |
|---------|------------|------|
| Teona Berozashvili | T.Berozashvili | 10/10/2025  |
| Ani Kelenjeridze |  |  |
| Saba Morchilashvili |  |  |
| Aleksi Tkebuchava |  |  |

---

## 🕓 Revision History

| Date | Change Made | Reason | Agreed By |
|------|--------------|---------|------------|
| [Date] | Initial contract | Lab 2 | All members |

---

**Last Updated:** [Date]  
**Stored at:** `/00-foundation/team-contract.md`

